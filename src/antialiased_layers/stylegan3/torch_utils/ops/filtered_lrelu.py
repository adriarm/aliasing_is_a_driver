...
#----------------------------------------------------------------------------
def quantile_adjust(inputs, q=None):
    if q is not None:
        batch_size, num_channels, _, _ = inputs.shape
        q_inputs = torch.quantile(inputs.view(
            batch_size, num_channels, -1), q=q, dim=-1)
        inputs = inputs + torch.nn.functional.relu(-q_inputs[:, :, None, None])
    return inputs
...
def _filtered_lrelu_ref(x, fu=None, fd=None, b=None, up=1, down=1, padding=0, gain=np.sqrt(2), slope=0.2, clamp=None, flip_filter=False):
    ...
    x = quantile_adjust(x, 0.4) # Hacky quantile   
